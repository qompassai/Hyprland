...
-- FIXME: regex patterns with backslash sequences (windowrule fix),
-- FIXME: already-converted .lua input passthrough.
-- FIXME: """
...
-- FIXME: def _has_invalid_lua_escape(s: str) -> bool:
i = 0
-- FIXME: while i < len(s):
-- FIXME: if s[i] == '\\' and i + 1 < len(s):
-- FIXME: if s[i + 1] not in _LUA_VALID_ESCAPES:
-- FIXME: return True
-- FIXME: i += 2
-- FIXME: else:
-- FIXME: i += 1
...
-- FIXME: if "'" in s:
-- FIXME: return True
-- FIXME: if _has_invalid_lua_escape(s):
-- FIXME: return True
-- FIXME: return False
...
-- FIXME: def convert_windowrule(s: str) -> str:
-- FIXME: """
-- FIXME: Convert windowrule/windowrulev2 = ... lines.
-- FIXME: Always uses Lua [[...]] long-string so regex backslash sequences
-- FIXME: like \\w \\s \\d \\. never trigger 'Invalid escape sequence'.
-- FIXME: """
m = [[re.match(r'^windowrulev?2?\s*=\s*(.+)$', s)]]
-- FIXME: if not m:
-- FIXME: return f'-- FIXME(windowrule): {s}'
rhs = 'm.group(1).strip()'
-- FIXME: return f'hl.windowrule({lua_string(rhs)})'
...
-- FIXME: def convert_windowrule_block(block_lines: list) -> list:
-- FIXME: """
-- FIXME: Convert a windowrule { name=... tile=... } config block into
-- FIXME: individual hl.windowrule() calls instead of a broken hl.config table.
-- FIXME: """
props = '{}'
name = 'None'
match_clauses = '[]'

-- FIXME: for line in block_lines:
s = 'line.strip()'
-- FIXME: if not s or s.startswith('#') or s.startswith('--'):
-- FIXME: continue
kv = [[re.match(r'^([\w.]+)\s*=\s*(.+)$', s)]]
-- FIXME: if not kv:
-- FIXME: continue
k = 'kv.group(1).strip()'
-- FIXME: v_raw, _ = strip_inline_comment(kv.group(2))
v = [[v_raw.strip().strip("'\"")]]
-- FIXME: if k == 'name':
name = 'v'
-- FIXME: elif k.startswith('match:') or k == 'match':
-- FIXME: match_clauses.append(f'{k} {v}')
-- FIXME: else:
-- FIXME: props[k] = v

out = '[]'
-- FIXME: if name:
-- FIXME: out.append(f'-- windowrule group: {name}')
-- FIXME: for prop_k, prop_v in props.items():
parts = [[[f'{prop_k} {prop_v}'] + match_clauses]]
rhs = [[', '.join(parts)]]
-- FIXME: out.append(f'hl.windowrule({lua_string(rhs)})')
...
-- FIXME: def _is_already_lua(lines: list) -> bool:
-- FIXME: """Return True if the file is already a converted .lua file (not a .conf)."""
-- FIXME: for line in lines[:10]:
s = 'line.strip()'
-- FIXME: if s.startswith('#!/usr/bin/env lua'):
-- FIXME: return True
-- FIXME: if re.match(r'^hl\.\w+\(', s):
-- FIXME: return True
-- FIXME: return False
...
-- FIXME: if src.suffix == '.lua' or _is_already_lua(lines):
-- FIXME: return '\n'.join(lines) + '\n'
...
-- FIXME: windowrule_blocks: list = []  # list of (name, body_lines)
i = 0
-- FIXME: while i < len(lines):
raw = 'lines[i]'
s = 'raw.strip()'
...
-- FIXME: if re.match(r'^windowrule\s*\{', s):
-- FIXME: body, i = collect_section(lines, i + 1)
-- FIXME: out.extend(convert_windowrule_block(body))
-- FIXME: continue
...
-- FIXME: i += 1
-- FIXME: continue
-- FIXME: if re.match(r'^windowrulev?2?\s*=', s):
-- FIXME: out.append(convert_windowrule(s))
-- FIXME: i += 1
-- FIXME: continue
